...
| `INTERNET` | Communicating with your own PC server on your local network. No external services are contacted. |
...
| Is data collected encrypted in transit? | Yes — optional TLS to your local PC; no external services are contacted |
| Do you provide a way for users to request data deletion? | Yes — uninstall the app (all data permanently deleted by Android) |
| Does the app share data with third parties? | No — no data is shared with any third party |
